...
public class hatShoot : MonoBehaviour
{
    public float shootSpeed = 7.5f;    //화살 속도
    /*public float shootDelay = 0.25f;    //발사 간격*/
    /*public GameObject bowPrefab;        //활의 프리펩*/
    public GameObject hatPrefab;      //호살의 프리펩
    ...
    {
        //모자을 가지고 있음 & 공격중이 아님
        if (ItemKeeper.hashats > 0 && inAttack == false)
        {
            ItemKeeper.hashats -= 1;	//화살을 소모
            ...
            GameObject arrowObj = Instantiate(hatPrefab, transform.position, r);
            //화살을 발사할 벡터 생성
            ...
            //공격 중이 아님으로 설정
            Invoke("StopAttack",1/* shootDelay*/);
